...
procedure InitRegCode(const Key: TKey; const RegStr: AnsiString; Expires: TDateTime; var Code:
  TCode);
...
procedure InitSerialNumberCode(const Key: TKey; Serial: LongInt; Expires: TDateTime; var Code:
  TCode);
...
function GenerateStringModifierPrimMg(const S: string): LongInt;
...
procedure ApplyModifierToKeyPrimMg(Modifier: Cardinal; var Key; KeySize: Cardinal);
...
{$IFNDEF NoMakeCodesSupport},
  OnGuard2{$ENDIF} {!!.10} {!!.10}
{$IFNDEF NoMakeKeysSupport},
  OnGuard3{$ENDIF} {!!.10} {!!.10}
  ;
...
begin
  ...
  try
    sRes := Format('%u', [CalculateCRC32(aStringStream)]);
    Result := StrToInt64(sRes); // Hex2Dec(sRes);
  finally
    aStringStream.Free;
  end;
end;
...
procedure MgXorMem(var Mem1; const Mem2: Cardinal; Count: Cardinal);
var
...
begin
  if Key[0] <> $FF then
    for Idx := 0 to Count - 1 do
      Key[idx] := Key[idx] xor Num[idx];
end;

procedure ApplyModifierToKeyPrimMg(Modifier: Cardinal; var Key; KeySize: Cardinal);
begin
  if Modifier <> 0 then
    MgXorMem(Key, Modifier, Min(SizeOf(Modifier), KeySize));
end;
...
begin
  ...
  if FStoreModifier then
    Result := FModifier
  else if Assigned(FOnGetModifier) then
  begin
...
begin
  ...
  if IsDateCodeValid(Key, Code) then
  begin
...
begin
  ...
  ApplyModifierToKeyPrim(Modifier, Key, SizeOf(Key));
  if IsDaysCodeValid(Key, Code) then
  begin
    if IsDaysCodeExpired(Key, Code) then
    begin
...
begin
  ...
  if IsUsageCodeValid(Key, Code) then
  begin
    if IsUsageCodeExpired(Key, Code) then
    begin
