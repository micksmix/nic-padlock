...
//This impersonation, which will work with WMI Scripting API calls but may constitute an unnecessary security risk, is supported only under Windows 2000.
  ...
  //Used for prototyping. It stops the query from happening and returns an object that looks like a typical result object.
...
{$IFDEF UNDEF}{$IFDEF UNDEF}{$ENDREGION}{$ENDIF}{$ENDIF}
  ...
  private
{$IFDEF WbemScripting_TLB}
    FSWbemLocator: TSWbemLocator; //ISWbemLocator;
...
{$ENDIF}
{$IFDEF WbemScripting_TLB}
    property SWbemLocator: TSWbemLocator read FSWbemLocator; //ISWbemLocator read FSWbemLocator;
...
{$ENDIF}
  protected
    ...
    function _LoadWmiData: boolean; overload;
    function _LoadWmiData(sSelectStmt, sSelectModifiers: string): boolean; overload;
...
{$IFDEF UNDEF}{$REGION 'Documentation'}{$ENDIF}
    ...
    /// </summary>
{$IFDEF UNDEF}{$ENDREGION}{$ENDIF}
    function GetCollectionIndexByPropertyValue(const PropertyName: string; AValue: OleVariant):
      integer;
      virtual;
{$IFDEF UNDEF}{$REGION 'Documentation'}{$ENDIF}
    /// <summary>
...
{$IFDEF UNDEF}{$ENDREGION}{$ENDIF}
    ...
    procedure LoadWmiData(const sSelect: string; const sSelectMod: string); overload;
    destructor Destroy; override;
...
{$ENDIF}
{$IFDEF WbemScripting_TLB}
    FSWbemLocator := TSWbemLocator.Create(nil); //CoSWbemLocator.Create;
    FWMIService := FSWbemLocator.ConnectServer(WmiServer, WmiNameSpace, WmiUser, WmiPass, '', '', 0,
      nil);
    if not FWmiIsLocal then
      FWMIService.Security_.ImpersonationLevel := wbemImpersonationLevelImpersonate;
{$ENDIF}
...
function TWmiClass.GetCollectionIndexByPropertyValue(const PropertyName: string; AValue:
  OleVariant):
  integer;
var
  i                           : Integer;
...
function TWmiClass._LoadWmiData(sSelectStmt, sSelectModifiers: string): boolean;
var
{$IFDEF WMI_LateBinding}
...
{$IFDEF _DEBUG}OutputDebugString(PChar('Connected ' + FormatDateTime('hh:nn:ss.zzz', Now - dt)));
{$ENDIF}
...
{$ELSE}
  //    objWbemObjectSet := FWmiConnection.FWMIService.ExecQuery(Format('SELECT * FROM %s', [FWmiClass]), 'WQL',
  //      wbemFlagForwardOnly or wbemFlagReturnImmediately);

  objWbemObjectSet := FWmiConnection.FWMIService.ExecQuery(Format('%s FROM %s %s', [sSelectStmt,
    FWmiClass, sSelectModifiers]), 'WQL', wbemFlagForwardOnly and wbemFlagReturnImmediately);
...
{$IFDEF WbemScripting_TLB}
  //    objWbemObjectSet := FWmiConnection.FWMIService.ExecQuery(Format('SELECT * FROM %s', [FWmiClass]), 'WQL',
  //      wbemFlagForwardOnly or wbemFlagReturnImmediately, nil);

  objWbemObjectSet := FWmiConnection.FWMIService.ExecQuery(Format('%s FROM %s %s', [sSelectStmt,
    FWmiClass,
      sSelectModifiers]), 'WQL', wbemFlagForwardOnly and wbemFlagReturnImmediately, nil);

  oEnum := (objWbemObjectSet._NewEnum) as IEnumVariant;
{$ENDIF}
{$IFDEF _DEBUG}OutputDebugString(PChar('Query Executed in ' + FormatDateTime('hh:nn:ss.zzz', Now -
    dt)));
{$ENDIF}
...
{$IFDEF _DEBUG}OutputDebugString(PChar('Pass in ' + FormatDateTime('hh:nn:ss.zzz', Now - dt)));
{$ENDIF}
  end;
{$IFDEF _DEBUG}OutputDebugString(PChar('Assigned in ' + FormatDateTime('hh:nn:ss.zzz', Now - dg)));
{$ENDIF}
  ...
  //    begin
  //      OutputDebugString(PChar('***** err: ' + E.Message));
  //      Result := False;
  //    end;
  //  end;
end;
...
{$IFDEF FPC}
    ...
    oEnum := IUnknown(objWbemObjectSet._NewEnum) as IEnumVariant;
{$ELSE}
    objWbemObjectSet := FWmiConnection.FWMIService.ExecQuery(Format('SELECT * FROM %s',
      [FWmiClass]),
      'WQL',
      wbemFlagForwardOnly or wbemFlagReturnImmediately);
    oEnum := IUnknown(objWbemObjectSet._NewEnum) as IEnumVariant;
...
procedure TWmiClass.LoadWmiData(const sSelect: string; const sSelectMod: string);
begin
  FWMiDataLoaded := _LoadWmiData(sSelect, sSelectMod);
end;
