...
type
  ...
  ISWbemObjectDisp = dispinterface
    ['{76A6415A-CB41-11D1-8B02-00600806D9B6}']
    function Put_(iFlags: Integer; const objWbemNamedValueSet: IDispatch): ISWbemObjectPath; dispid
      1;
    procedure PutAsync_(const objWbemSink: IDispatch; iFlags: Integer;
  ...
  ISWbemSecurity = interface(IDispatch)
    ...
    property ImpersonationLevel: WbemImpersonationLevelEnum read Get_ImpersonationLevel write
      Set_ImpersonationLevel;
    property AuthenticationLevel: WbemAuthenticationLevelEnum read Get_AuthenticationLevel write
  ...
  ISWbemQualifier = interface(IDispatch)
    ...
    property PropagatesToSubclass: WordBool read Get_PropagatesToSubclass write
      Set_PropagatesToSubclass;
    property PropagatesToInstance: WordBool read Get_PropagatesToInstance write
      Set_PropagatesToInstance;
  ...
  ISWbemDateTime = interface(IDispatch)
    ...
    property MicrosecondsSpecified: WordBool read Get_MicrosecondsSpecified write
      Set_MicrosecondsSpecified;
...
function TSWbemLastError.Put_(iFlags: Integer; const objWbemNamedValueSet: IDispatch):
  ISWbemObjectPath;
begin
...
function TSWbemLastError.Instances_(iFlags: Integer; const objWbemNamedValueSet: IDispatch):
  ISWbemObjectSet;
begin
...
function TSWbemLastError.Subclasses_(iFlags: Integer; const objWbemNamedValueSet: IDispatch):
  ISWbemObjectSet;
begin
  ...
  case DispID of
    ...
    1: if Assigned(FOnObjectReady) then
        FOnObjectReady(Self,
          ...
          IUnknown(TVarData(Params[1]).VPointer) as ISWbemNamedValueSet
            {const ISWbemNamedValueSet});
    2: if Assigned(FOnCompleted) then
        FOnCompleted(Self,
          ...
          IUnknown(TVarData(Params[2]).VPointer) as ISWbemNamedValueSet
            {const ISWbemNamedValueSet});
    3: if Assigned(FOnProgress) then
        FOnProgress(Self,
          ...
          IUnknown(TVarData(Params[3]).VPointer) as ISWbemNamedValueSet
            {const ISWbemNamedValueSet});
...
class function CoSWbemRefreshableItem.CreateRemote(const MachineName: string):
...
procedure Register;
begin
  RegisterComponents(dtlServerPage, [TSWbemLocator, TSWbemNamedValueSet, TSWbemObjectPath,
    TSWbemLastError,
