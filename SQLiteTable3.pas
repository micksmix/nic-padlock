...
type
  ...
  public
    ...
    function Backup(TargetDB: TSQLiteDatabase; targetName: string; sourceName: string): integer;
      overload;
...
begin
  ...
  if Msg <> nil then
    raise ESqliteException.CreateFmt(s + '.'#13'Error [%d]: %s.'#13'"%s": %s', [ret,
      SQLiteErrorStr(ret),
        SQL, Msg])
  else
...
begin
  if Assigned(Query.Statement) then
    Sqlite3_Bind_Text16(Query.Statement, Index, PChar(Value), Length(Value) * SizeOf(char),
...
function TSQLiteDatabase.Backup(TargetDB: TSQLiteDatabase; targetName: string; sourceName: string):
  integer;
var
  pBackup                     : TSQLiteBackup;
begin
  pBackup := Sqlite3_backup_init(TargetDB.DB, PAnsiChar(ansistring(targetName)), self.DB,
    PAnsiChar(ansistring(sourceName)));
